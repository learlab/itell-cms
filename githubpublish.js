...
const pagesQuery =
  "?populate[0]=Pages.Content&populate[1]=Pages.Chapter&sort[0]=Pages.Order";
const chaptersQuery =
...
async function parseChapters(textData) {
  ...
  for (let chapterIndex in textData) {
    ...
    chapter["Pages"] = chapter["Pages"].sort((a, b) => a["Order"] - b["Order"]);
    for (let i in chapter["Pages"]) {
      let page = chapter["Pages"][i];
      await writePage(
        page,
        `  title: ${chapter["Title"]}\n  slug: ${chapter["Slug"]}`,
        parseInt(i) === chapter["Pages"].length - 1
          ? nextSlug
          : chapter["Pages"][parseInt(i) + 1]["Slug"],
      );
    }
  }
}
    ...
    if (chunk["__component"] === "page.video") {
      ...
      if (matches !== null) {
        for (let match of matches) {
          chunk["MDX"] = chunk["MDX"].replace(
            match,
            match +
              `{#${match.replaceAll("#", "").toLowerCase().trim().replaceAll(" ", "-")}}`,
          );
        }
      }
      ...
      if (matches !== null) {
        for (let match of matches) {
          chunk["MD"] = chunk["MD"].replace(
            match,
            `${match} {#${match.replaceAll("#", "").toLowerCase().trim().replaceAll(" ", "-")}}`,
          );
        }
      }
...
async function createGuide() {
  const guideRes = await fetch(
    `${URL}texts/${userGuideDocumentID}?populate[0]=Pages.Content`,
    {
      cache: "no-store",
    },
  );
...
async function run() {
  ...
  if (hasChapters) {
    ...
    let data = await res.json();
    const pages = data["data"]["Pages"].sort((a, b) => a["Order"] - b["Order"]);
    for (let i in pages) {
      let page = pages[i];
      await writePage(
        page,
        "null",
        parseInt(i) === pages.length - 1
          ? "null"
          : pages[parseInt(i) + 1]["Slug"],
      );
    }
  }
